...
library(gplots)     # provides plotCI
...
#postPhotogeo <- postPhotogeo/max(postPhotogeo)
...
photgeo.dist.prior <- function(r,rlen,beta,alpha,p,phot_g_mean_mag,bp_rp,pseudocolour){
    ...
    #cat(rlen,beta,alpha,p,phot_g_mean_mag,bp_rp,pseudocolour,"\n")
    #cat(range(r),"\n")
    ...
    #lnprior <- beta*log(r) - (r/rlen)^alpha + log(interpolate.QGmodel.density(r=r, gmag=phot_g_mean_mag, bprp=bp_rp, QGmodel1=QGmodel1, QGmodel2=QGmodel2, bprpofQGmodels=bprpofQGmodels))
